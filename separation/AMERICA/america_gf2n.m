function B = america_gf2n(PPx, P, K, PK, Lex, r, gf_exp)
...
eqeps = 1e-11; %a threshold for deciding
              %equal entropies
...
h = -sum(ffq .* log2(ffq + eps)/log2(P), 1);
...
while k <= K
    ...
    %% test if the selected is not a linear combination of the previous ones(?)
    ...
    if ~any( sum(test0.x, 1) == 0) %not a linear combination
